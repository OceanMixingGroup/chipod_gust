% [merged] = merge_cell_structs(in, concat_2d, concat_dim)
...
function [merged] = merge_cell_structs(in, concat_2d, concat_dim)
    ...
    for f=1:length(F)
        if isstruct(in{1}.(F{f}))
            ...
            if isfield(mat(1), 'time')
                lentime = length(mat(1).time);
                indtime = find(size(mat(1).(F{f})) == lentime);
                if ~isempty(indtime)
                    concat_dim = indtime;
                end
            end 
